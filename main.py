...
def req_csv():
    ...
    # Blue (likely / near) -> deep indigo (unlikely / far), with the top path in bright orange-white
    start_rgb = (111, 168, 255)   # BLUE
    end_rgb = (40, 46, 74)        # deep indigo, fades into the panel
    ...
    for rank, (prob, path) in enumerate(top_paths):
        ...
        is_top = rank == 0
        line_color = ORANGE if is_top else color
        ...
        global_fig.add_trace(go.Scatter(
            ...
            y=prices,
            mode='lines',
            line=dict(color=line_color, width=4 if is_top else 1.4),
